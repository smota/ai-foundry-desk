...
6. promotes the verified version:
   ...
   ```powershell
   npm dist-tag add ai-foundry-desk@0.8.0 latest
   ```

Only after promotion may documentation advertise npm as the default convenience channel. Verify
that `npm view ai-foundry-desk dist-tags --json` reports `latest` as `0.8.0` and that GitHub's latest
release is `v0.8.0`; a prerelease must not remain the canonical latest version. The checksummed
