...
test("help and version use the afd brand", () => {
  ...
  assert.equal(version.status, 0); assert.equal(version.stdout.trim(), "0.9.0");
});
...
test("doctor and fix expose safe argument contracts", () => {
  ...
  assert.notEqual(fix.status, 0); assert.match(fix.stderr, /Usage: afd fix sandbox/);
...
test("provenance identifies the running CLI and hybrid repair fails closed", () => {
  const provenance = cli("provenance", "--json"); const repair = cli("fix", "sandbox", "--dry-run");
  ...
  assert.equal(value.version, "0.9.0"); assert.match(value.cli ?? "", /cli\.js$/);
