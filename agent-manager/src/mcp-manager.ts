...
export function renderMcpPlan(plan: McpPlan): string { const rows = [`AFD MCP ${plan.kind} plan`, "", `[${plan.blocked ? "BLOCKED" : "READY"}] ${plan.actions.length} file action(s) planned`, `  Scope: ${plan.scope}`, `  Project: ${plan.project ?? "-"}`, `  Approval token: ${plan.approvalToken}`, "", "Actions:"]; for (const action of plan.actions) rows.push(`- ${action.kind.toUpperCase()} ${action.agent}/${action.scope}`, `  Path: ${action.path}`, `  Detail: ${action.detail}`); if (plan.blockers.length) rows.push("", "Blockers:", ...plan.blockers.map((item) => `- ${item}`)); return `${rows.join("\n")}\n`; }
