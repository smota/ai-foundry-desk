...
import { executeProject, projectDoctor } from "./project-environment.js";
import { applyHostPlan, planHostLayer, recoverHostLayer, renderHostPlan, renderHostVerification, rollbackHostLayer, verifyHostLayer } from "./host-lifecycle.js";
...
Workstation:
  afd layer1 <command>               Plan, apply, verify, recover, or roll back foundation setup
  afd layer2 <command>               Plan, apply, verify, recover, or roll back agent-tool setup
...
async function dispatch(args: readonly string[]): Promise<number> {
  ...
  if (command === "init") { if (args.slice(1).some((arg) => arg !== "--dry-run")) throw new Error("Usage: afd init [--dry-run]"); console.log("AI Foundry Desk is ready for inspection. No layer was applied."); console.log("Next: afd status, then afd layer1 plan and afd layer2 plan."); return 0; }
  ...
  if (command === "doctor" && args.includes("--project")) {
  ...
  if (command === "fix" && args[1] === "rust") {
    if (args.length !== 3 || !["--dry-run", "--apply"].includes(args[2]!)) throw new Error("Usage: afd fix rust --dry-run|--apply");
    if (process.platform !== "win32") throw new Error("The MSVC prerequisite repair is Windows-only.");
  ...
  if (command === "fix") { const target=args[1];if(target!=="sandbox"||args.slice(2).some((arg) => !["--apply", "--dry-run"].includes(arg)))throw new Error("Usage: afd fix sandbox --dry-run|--apply");const apply=args.includes("--apply");const dryRun=args.includes("--dry-run");if(apply===dryRun)throw new Error("Use exactly one option: afd fix sandbox --dry-run or --apply.");const identity=await executionIdentity();if(identity.mismatch||identity.context==="sandbox")throw new Error("Sandbox access repair is refused because the process token and profile identity do not match. Use a normal shell for the intended user.");if(process.platform!=="win32")throw new Error("Sandbox access repair is Windows-only.");const repaired=await runPowerShellArgs("13-reconcile-sandbox-toolchain-access.ps1",["-Mode",dryRun?"Plan":"Apply",...(apply?["-Approved"]:[])]);if(repaired!==0||dryRun)return repaired;return runPowerShellArgs("13-reconcile-sandbox-toolchain-access.ps1",["-Mode","Plan"]); }
  if (command === "layer1" || command === "layer2") {
    const layer = command === "layer1" ? 1 : 2;
    const subcommand = args[1];
    const json = args.includes("--json");
    const confirmIndex = args.indexOf("--confirm");
    const confirm = confirmIndex >= 0 ? args[confirmIndex + 1] : undefined;
    const allowed = new Set(["--json", "--confirm"]);
    for (let index = 2; index < args.length; index++) {
      const value = args[index]!;
      if (!allowed.has(value) && args[index - 1] !== "--confirm") throw new Error(`Unknown ${command} option: ${value}`);
    }
    if (subcommand === "plan") {
      if (confirm) throw new Error(`Usage: afd ${command} plan [--json]`);
      const plan = await planHostLayer(layer);
      console.log(json ? JSON.stringify(plan, null, 2) : renderHostPlan(plan));
      return plan.blocked ? 2 : 0;
    }
    if (subcommand === "verify") {
      if (confirm) throw new Error(`Usage: afd ${command} verify [--json]`);
      const report = await verifyHostLayer(layer);
      console.log(json ? JSON.stringify(report, null, 2) : renderHostVerification(report));
      return report.valid ? 0 : 2;
    }
    if (!["apply", "recover", "rollback"].includes(subcommand ?? "") || !confirm) throw new Error(`Usage: afd ${command} plan|verify [--json] | apply|recover|rollback --confirm <plan-token> [--json]`);
    const result = subcommand === "apply"
      ? await applyHostPlan(await planHostLayer(layer), confirm)
      : subcommand === "recover"
        ? await recoverHostLayer(layer, confirm)
        : await rollbackHostLayer(layer, confirm);
  ...
  if (["status", "review", "verify"].includes(command)) { if (args.length !== 1) throw new Error(`Usage: afd ${command}`); const result = await inspect({ dryRun: true }); print(result.changes); if(command==="review"){const pendingItems=await listPending(result.root);if(pendingItems.length)console.log(renderItems("AFD pending skills",pendingItems.map(item=>`${item.agent}/${item.id} - ${item.path}`)));} const pending = result.changes.some((change) => ["drift", "create", "update"].includes(change.kind)); if(command==="verify"){for(const layer of [1,2] as const){const report=await verifyHostLayer(layer);console.log(renderHostVerification(report));if(!report.valid)return 2;}if(process.platform==="win32"){const code=await runPowerShell("10-verify-backups.ps1");if(code!==0)return code;}} return command === "verify" && pending ? 2 : 0; }
