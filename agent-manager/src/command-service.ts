...
async function dispatch(args: readonly string[]): Promise<number> {
  ...
  if (command === "layer1" || command === "layer2") {
    ...
    if(process.platform==="win32"){if(allowClaude)throw new Error("Claude postinstall opt-in is Linux-only.");const scripts=command==="layer1"?["01-layer1-runtime.ps1","02-docker-windows.ps1"]:["07-layer2-agent-clis.ps1","07-layer2-common-toolbox.ps1"];for(const script of scripts){const code=await runPowerShell(script,dryRun);if(code!==0)return code;}return 0;}
