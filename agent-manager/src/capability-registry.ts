...
export const capabilityRegistry: readonly CapabilityDefinition[] = [
  ...
  c("Overview", "verify", "Full verification", "verify", "Inspect catalog drift and verify both host layers through the typed capability contracts.", "read-only", "verify"),

  c("Workstation", "layer1-plan", "Plan Layer 1", "layer1 plan", "Plan runtimes, package managers, supply-chain guardrails, and host tools.", "read-only", "plan"),
  c("Workstation", "layer1-apply", "Apply Layer 1", "layer1 apply --confirm <plan-token>", "Apply the exact reviewed Layer 1 plan.", "writes-local", "apply", "Plan token"),
  c("Workstation", "layer1-verify", "Verify Layer 1", "layer1 verify", "Verify live commands against ownership and compatibility contracts.", "read-only", "verify"),
  c("Workstation", "layer1-recover", "Recover Layer 1", "layer1 recover --confirm <plan-token>", "Resume after an interrupted or failed apply using its receipt.", "writes-local", "apply", "Interrupted plan token"),
  c("Workstation", "layer1-rollback", "Retire Layer 1 receipt", "layer1 rollback --confirm <plan-token>", "Retire AFD-owned operation state without uninstalling user-owned tools.", "destructive-recoverable", "rollback", "Applied plan token"),
  c("Workstation", "layer2-plan", "Plan Layer 2", "layer2 plan", "Plan supported agent applications, CLIs, and common toolbox.", "read-only", "plan"),
  c("Workstation", "layer2-apply", "Apply Layer 2", "layer2 apply --confirm <plan-token>", "Apply the exact reviewed Layer 2 plan.", "writes-local", "apply", "Plan token"),
  c("Workstation", "layer2-verify", "Verify Layer 2", "layer2 verify", "Verify live Layer 2 capabilities and provenance.", "read-only", "verify"),
  c("Workstation", "layer2-recover", "Recover Layer 2", "layer2 recover --confirm <plan-token>", "Resume after an interrupted or failed Layer 2 apply.", "writes-local", "apply", "Interrupted plan token"),
  c("Workstation", "layer2-rollback", "Retire Layer 2 receipt", "layer2 rollback --confirm <plan-token>", "Retire AFD-owned operation state without uninstalling user-owned tools.", "destructive-recoverable", "rollback", "Applied plan token"),
