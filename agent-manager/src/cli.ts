...
async function main(args: readonly string[]): Promise<number> {
  ...
  if (command === "layer3" && args[1] === "recipes") { console.log("builtin:samuel\nbuiltin:observability"); return 0; }
  ...
  if (command === "telemetry") {
    ...
    if (sub === "apply") { const confirmIndex = args.indexOf("--confirm"); if (confirmIndex < 0 || !args[confirmIndex + 1]) throw new Error("Use the token from telemetry plan: afd telemetry apply --confirm <plan-token>."); console.log(JSON.stringify(await applyRecipe(source, { confirm: true, approvalToken: args[confirmIndex + 1] }), null, 2)); return 0; }
    if (sub === "status") { const status = await telemetryStatus(); console.log(JSON.stringify(status, null, json ? 2 : 0)); return status.state === "healthy" || status.state === "disabled" ? 0 : 2; }
    if (sub === "verify") { console.log(JSON.stringify(await verifyTelemetry(), null, 2)); return 0; }
    if (sub === "stop") { await stopTelemetry(); console.log("STOPPED\ttelemetry-v2"); return 0; }
    if (sub === "uninstall-autostart") { await uninstallTelemetryAutostart(); console.log("AUTOSTART_REMOVED\ttelemetry-v2"); return 0; }
    if (sub === "resume") { const status=await resumeTelemetry();console.log(JSON.stringify(status));return status.state==="healthy"?0:2; }
    if (sub === "refresh" && args[2] === "--agentacct" && args.length === 3) { const status = await telemetryStatus(); if (!status.agentacct.version) throw new Error("agentacct is not configured."); await new AgentacctAdapter(cliPlatform, status.agentacct.version).refresh(); console.log("REFRESHED\tagentacct"); return 0; }
