...
async function hermesSkills(home: string): Promise<string | undefined> {
  ...
  const managed = path.join(home, ".afd", "managed", "hermes", "skills"); if (await exists(managed)) return managed;
...
async function targetPaths(home: string): Promise<Record<AgentId, { skill?: string | undefined; profile?: string | undefined }>> {
  ...
  return { "claude-code": { skill: path.join(home, ".claude", "skills"), profile: path.join(home, ".claude", "CLAUDE.md") }, codex: { skill: shared, profile: path.join(home, ".codex", "AGENTS.md") }, antigravity: { skill: path.join(home,".gemini","antigravity-cli","skills") }, pi: { skill: shared, profile: path.join(home, ".pi", "agent", "AGENTS.md") }, hermes: { skill: await hermesSkills(home) }, grok: { skill: shared } };
}
...
async function ensureCanonical(root: string, dryRun: boolean, changes: Change[]): Promise<void> {
  ...
  for (const [target, content] of files) { const current = await text(target); if (current === undefined) { changes.push({ agent: "canonical", kind: "create", path: target, detail: "canonical base is missing" }); if (!dryRun) { await mkdir(path.dirname(target), { recursive: true }); await writeFile(target, content, "utf8"); } } else if (current === content || (target.endsWith("manifest.json") && sameJson(current, content))) { /* reconciled */ } else if (target.endsWith(`${SKILL_ID}\\SKILL.md`) && isPreviousManagedSkill(current)) { changes.push({ agent: "canonical", kind: "update", path: target, detail: "managed canonical revision" }); if (!dryRun) { await backupFile(root, "canonical", target, current); await writeFile(target, content, "utf8"); } } else if (target.endsWith(`${SKILL_ID}\\SKILL.md`)) changes.push({ agent: "canonical", kind: "drift", path: target, detail: "canonical skill is divergent or missing the toolbox policy" }); else if (target.endsWith("manifest.json")) changes.push({ agent: "canonical", kind: "drift", path: target, detail: "existing manifest preserved" }); }
}
async function copySkill(source: string, id: string, targetRoot: string, agent: AgentId, root: string, dryRun: boolean, changes: Change[]): Promise<void> {
  const target = path.join(targetRoot, id); const storedCanonical = await text(path.join(source, "SKILL.md")); if(storedCanonical===undefined){changes.push({agent,kind:"drift",path:source,detail:"Canonical skill is missing; destination preserved."});return;} const canonical = id===SKILL_ID&&isPreviousManagedSkill(storedCanonical) ? SKILL : storedCanonical; const current = await text(path.join(target, "SKILL.md"));
  if (id===SKILL_ID&&!hasToolboxPolicy(canonical)) { changes.push({ agent, kind: "drift", path: target, detail: "Canonical toolbox policy is incomplete; destination preserved." }); return; }
  ...
  else if (id===SKILL_ID&&isPreviousManagedSkill(current)) { changes.push({ agent, kind: "update", path: target, detail: "approved skill revision" }); if (!dryRun) { const targetFile = path.join(target, "SKILL.md"); await backupFile(root, agent, targetFile, current); await writeFile(targetFile, canonical, "utf8"); } }
...
export async function inspect(options: ManagerOptions = {}): Promise<ManagerResult> {
  ...
  const manifestPath = path.join(root, "manifest.json"); const manifest = await exists(manifestPath) ? await loadManifest(manifestPath) : MANIFEST; await ensureCanonical(root, dryRun, changes); const map = await targetPaths(home); const profileSource = path.join(root, manifest.profile.source);
  for (const target of agentTargets) { const selection = manifest.targets.find((item) => item.agent === target.id); const destination = map[target.id]; if (target.skills !== "supported" || !destination.skill) changes.push({ agent: target.id, kind: "unsupported", path: "-", detail: target.reason ?? "skills unavailable" }); else for(const id of selection?.entries??[]){const entry=manifest.catalog.find(item=>item.id===id);if(entry)await copySkill(path.join(root,entry.source),id,destination.skill,target.id,root,dryRun,changes);} if(destination.skill)try{for(const entry of await readdir(destination.skill,{withFileTypes:true}))if(entry.isDirectory()&&!selection?.entries.includes(entry.name))changes.push({agent:target.id,kind:"importable",path:path.join(destination.skill,entry.name),detail:"unmanaged direct skill"});}catch{/* absent */} if (selection?.profile && target.profile === "supported" && destination.profile) await mergeProfile(profileSource, destination.profile, target.id, root, dryRun, changes); else if (target.profile !== "supported") changes.push({ agent: target.id, kind: "unsupported", path: "-", detail: `profile: ${target.reason ?? "unsupported"}` }); }
  return { root, changes };
}
