...
export function nativeMcpPath(agent: AgentId, scope: McpScope, options: McpManagerOptions): string {
  ...
  switch (agent) {
    ...
    case "antigravity": return path.join(root, ".gemini", "settings.json");
...
function normalizeRaw(raw: unknown, id: string, targets: readonly AgentId[], agent: AgentId): McpServer {
  ...
  if (typeof raw.url === "string") {
    ...
    return { transport: "http", url: raw.url, enabled, targets, ...((headers || envHeaders) ? { headers: { ...headers, ...envHeaders } } : {}) };
  }
...
export function renderNativeMcpContent(agent: AgentId, id: string, server: McpServer | null, before: string): string {
  ...
  if (agent === "pi") {
    ...
    const servers = plain(parsed.mcpServers) ? { ...parsed.mcpServers } : {}; if (server) servers[id] = { ...rawJson(server), disabled: !server.enabled }; else delete servers[id]; parsed.mcpServers = servers; return `${JSON.stringify(parsed, null, 2)}\n`;
  }
