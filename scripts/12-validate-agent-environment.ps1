...
if ($globalAfd) {
    ...
    if ($globalProvenance.Status -eq 0) {
        ...
        $globalProvenancePass = $globalDetails.version -eq "0.8.0" -and -not [string]::IsNullOrWhiteSpace($globalDetails.cli) -and -not [string]::IsNullOrWhiteSpace($globalDetails.runtime.executable)
