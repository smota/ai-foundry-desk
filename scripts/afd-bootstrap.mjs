...
const defaultPrefix = process.platform === "win32" ? path.join(process.env.LOCALAPPDATA ?? path.join(homedir(), "AppData", "Local"), "AI Foundry Desk", "cli") : path.join(homedir(), ".local");
const version = option("--version", "0.9.0"); const repository = option("--repository", "smota/ai-foundry-desk"); const prefix = option("--prefix", defaultPrefix); const assetDir = option("--asset-dir", undefined);
...
async function installWindowsLauncher(globalDir, bin) {
  let packageRoot;
  ...
  const cli = path.join(packageRoot, "agent-manager", "dist", "cli.js");
...
try {
  ...
  const globalDir = path.join(prefix, "share", "ai-foundry-desk", "versions", version, "global");
  const bin = path.join(prefix, "bin");
  const isolation = ["--global-dir", globalDir, "--global-bin-dir", bin, "--store-dir", path.join(prefix, "share", "ai-foundry-desk", "store")];
  const args = ["add", "--global", ...isolation, "--ignore-scripts", ...(assetDir ? ["--offline"] : []), packagePath];
  await mkdir(bin, { recursive: true }); process.env.PATH = bin + path.delimiter + (process.env.PATH ?? "");
  await run(pnpm, args); if (process.platform === "win32") await installWindowsLauncher(globalDir, bin); console.log("AI Foundry Desk " + version + " installed. No Layer was applied.");
