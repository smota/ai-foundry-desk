...
async function prepareOutput() {
  ...
  const generated = /^(?:ai-foundry-desk-[0-9].*\.tgz(?:\.sha256)?|afd-bootstrap(?:-posix|-windows)?\.(?:mjs|ps1|sh)(?:\.sha256)?|RELEASE[-_]NOTES\.md)$/;
  for (const entry of await readdir(output, { withFileTypes: true })) {
    if (!entry.isFile() || !generated.test(entry.name)) throw new Error(`Release output contains unknown content and was not cleaned: ${entry.name}`);
